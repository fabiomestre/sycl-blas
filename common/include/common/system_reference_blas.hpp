...
template <>
struct BlasSystemFunction<sycl_complex<float>> {
  template <typename floatfn_t, typename doublefn_t, typename complexfloatfn_t, typename complexdoublefn_t>
...
template <>
struct BlasSystemFunction<sycl_complex<double>> {
  template <typename floatfn_t, typename doublefn_t, typename complexfloatfn_t, typename complexdoublefn_t>
...
scalar_t asum(const int n, scalar_t x[], const int incX) {
  auto func = blas_system_function<scalar_t>(&cblas_sasum, &cblas_dasum, &cblas_scasum, &cblas_dzasum);
...
void axpy(const int n, scalar_t alpha, const scalar_t x[], const int incX,
  ...
  if constexpr (std::is_same_v<scalar_t, sycl_complex<float>> || std::is_same_v<scalar_t, sycl_complex<double>>) {
